 ...
 * 城市搜索工具
 */
export function createCitySearchTool(weatherService) {
    return {
        name: 'search-city',
        description: '【重要】搜索中国城市，获取城市的经纬度坐标和ID。这是所有天气查询的前置步骤：在调用任何天气相关工具（get-current-weather、get-weather-forecast、get-weather-warnings）之前，必须先调用此工具获取目标城市的准确坐标。',
        inputSchema: {
            cityName: z
                .string()
                .min(2)
                .describe('城市名称（必须提供）。支持格式：北京、上海、广州、深圳、成都、杭州、南京、武汉、西安、重庆等中国城市名称'),
        },

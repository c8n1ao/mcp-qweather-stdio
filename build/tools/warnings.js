 ...
 * 天气预警查询工具
 */
export function createWarningsTool(weatherService) {
    return {
        name: 'get-weather-warnings',
        description: '获取指定城市的天气预警信息。【重要】使用前必须先调用 search-city 工具获取目标城市的准确经纬度坐标，然后使用获得的坐标（格式：经度,纬度，如"116.4074,39.9042"）作为location参数。',
        inputSchema: {
            location: z
                .string()
                .min(2)

...
import type { ForecastParams } from '../types/weather.js'

/**
 * 天气预报查询工具
 */
export function createForecastTool(weatherService: QWeatherService) {
  return {
    name: 'get-weather-forecast',
    description:
      '获取指定城市的天气预报（未来3-7天）。【重要】使用前必须先调用 search-city 工具获取目标城市的准确经纬度坐标，然后使用获得的坐标（格式：经度,纬度，如"116.4074,39.9042"）作为location参数。',
    inputSchema: {
      location: z
        .string()
        .min(2)
      ...
      days: z
        ...
        .default(3)
        .describe(
          '预报天数，1-7天之间的整数，默认为3天。例如：3表示未来3天，7表示未来7天'
        ),
    },
    handler: async ({ location, days = 3 }: ForecastParams) => {
